...
# Core Kibo APIs -------------------------------------------------------------
#  - ServoController : controls the 4 hobby servos in Kibo’s neck/body.
#  - face            : high‑level API for the LCD face (expression, blink, look).

from kibo import ServoController  # make sure kibo.py is on your PYTHONPATH
...
def nod(ctl: ServoController, down: int = 20, up: int = 60, t: float = 0.6):
...
def sway_left_right(ctl: ServoController, angle: int = 60, t: float = 0.4):
...
def ear_wiggle(ctl: ServoController, repeats: int = 4, t: float = 0.2):
    """Wiggle ears (servo 2)."""
...
def scene_wake_up(face_mod, ctl: ServoController):
...
def scene_meh(face_mod, ctl: ServoController):
...
def scene_happy_dance(face_mod, ctl: ServoController, bars: int = 4):
